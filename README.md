...
2. **Recuperación según la fuente.**
   - *Vectorial:* búsqueda híbrida (semántica en ChromaDB + BM25) seguida de un *re-ranking* con un modelo *cross-encoder*.
...
4. **Agente ReAct.** Como extensión, un agente selecciona de forma autónoma entre cuatro herramientas (`doc_search`, `table_search`, `graph_search` y `analytics_tool`), ejecuta la acción, observa el resultado y elabora la respuesta.
...
Ambos clasificadores de intención alcanzan una exactitud del 100 % sobre el conjunto de prueba. Cabe señalar que dicho conjunto es reducido (36 ejemplos sintéticos, 11 de prueba), por lo que el resultado debe interpretarse como una validación funcional y no como una medida de generalización.

Las respuestas generadas se apoyan exclusivamente en el contexto recuperado, incluyendo opiniones mixtas de los usuarios cuando corresponde, sin introducir información externa.
